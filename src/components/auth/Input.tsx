...
const Input = ({ state, onChange,placeholder,value,classNames,isLoading }:{state:state; onChange?:React.ChangeEventHandler<HTMLInputElement>;value:string;placeholder:string,classNames?:classNames,isLoading?:boolean}) => {
    const getColor = () => {
        switch(state){
            case 'normal':
  ...
  return (
    <div style={{borderColor:getColor()}} className={` ${classNames?.wrapper}  min-h-[50px] flex items-center justify-between  border-b-[1px]`}>
        <div  className="w-full h-full  flex items-center justify-start ">
            <User color={getColor()} size={25} className='mx-2' />
