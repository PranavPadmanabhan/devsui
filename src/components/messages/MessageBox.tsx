...
const MessageBox = ({ name, lastMessage,onClick,isActive}:{name:string;lastMessage?:string;onClick?:() => void,isActive?:boolean}) => {
  return (
    <div onClick={onClick} className={`relative w-[92%] min-h-[65px] rounded-[10px] bg-messageBoxBG border-[1px] ${isActive?'border-blue-600':'border-primaryBorder'} mb-2 flex items-center justify-between px-[4%] box-border`}>
